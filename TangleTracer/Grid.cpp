...
Grid::Grid(const Grid& grid): Compound(),
		nx(grid.nx),
		ny(grid.ny),
		nz(grid.nz),
		mesh_ptr(grid.mesh_ptr),
		reverse_normal(grid.reverse_normal)		
{
	//copy_objects(grid.objects);
}
...
//This function has been fixed at 5/9/2012, before it didn't perform a right shadow

bool 																						 
Grid::shadow_hit(const Ray& ray, float& tmin) const
{
	float t = 10000;
	...
	while (true) {	
		...
		if (tx_next < ty_next && tx_next < tz_next) {
			if (object_ptr && object_ptr->shadow_hit(ray, t) && t < tmin) {		//This part is different from hit function, Notice it!
				tmin = t;
				return (true);
			}
			...
			if (ty_next < tz_next) {
				if (object_ptr && object_ptr->shadow_hit(ray, t) && t < tmin) {
					tmin = t;
					return (true);
				}
