...
Rosette::Rosette(int num_rings, const float hole_r, const float ring_w, const float rb, const float y0, const float y1)
			: Grid() {
...
Rosette::Rosette (const Rosette& c)
	: Grid(c) {
...
void
Rosette::copy_objects(const vector<GeometricObject*>& rhs_ojects) {
	delete_objects();    	
