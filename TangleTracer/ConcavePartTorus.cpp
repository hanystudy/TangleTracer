...
ConcavePartTorus::ConcavePartTorus (const double _a, const double _b, const double _min_azimuth, const double _max_azimuth, const double _min_polar, const double _max_polar)
	: 	GeometricObject(),
		a(_a),
		b(_b),
		bbox(-a - b, a + b, -b, b, -a - b, a + b),
...
bool 																						 
ConcavePartTorus::shadow_hit(const Ray& ray, float& tmin) const
{
	if (!bbox.hit(ray))
		return (false);
